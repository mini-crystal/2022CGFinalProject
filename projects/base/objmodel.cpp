...
ObjModel::ObjModel(const std::string& filepath) {
    ...
    for (const auto& index : indexs) {
        ...
        vertex.position.x = attrib.vertices[3 * index.vertex_index + 0 - 3];
        vertex.position.y = attrib.vertices[3 * index.vertex_index + 1 - 3];
        vertex.position.z = attrib.vertices[3 * index.vertex_index + 2 - 3];

        if (index.normal_index >= 0) {
            vertex.normal.x = attrib.normals[3 * index.normal_index + 0 - 3];
            vertex.normal.y = attrib.normals[3 * index.normal_index + 1 - 3];
            vertex.normal.z = attrib.normals[3 * index.normal_index + 2 - 3];
        }

        if (index.texcoord_index >= 0) {
            vertex.texCoord.x = attrib.texcoords[2 * index.texcoord_index + 0 - 2];
            vertex.texCoord.y = attrib.texcoords[2 * index.texcoord_index + 1 - 2];
        }
...
ObjModel::ObjModel(const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices)
...
BoundingBox ObjModel::getBoundingBox() const {
    return _boundingBox;
}
...
void ObjModel::drawBoundingBox() const {
...
GLuint ObjModel::getVao() const {
    return _vao;
}

GLuint ObjModel::getBoundingBoxVao() const {
    return _boxVao;
}
...
size_t ObjModel::getFaceCount() const {
    return _indices.size() / 3;
}

void ObjModel::initGLResources() {
...
void ObjModel::initBoxGLResources() {
