...
void TextureMapping::initBlendShader() {
	...
	const char* fsCode =
		...
		"	vec3 diffuse = light.color * max(dot(lightDir, normal), 0.0f) * material.kds[0] * 0.5 + light.color * max(dot(lightDir, normal), 0.0f) * material.kds[1] * 0.5;\n"
		...
		"	color = mix(texture(mapKds[0], fTexCoord), texture(mapKds[1], fTexCoord), material.blend ) + vec4(diffuse, 1.0f) * 0.2;\n"
		"}\n";
	//----------------------------------------------------------------
...
void TextureMapping::initCheckerShader() {
	...
	const char* fsCode = 
		...
		"	color = vec4(material.colors[0], 1.0f) * vec4(fTexCoord, 1.0f, 1.0f) + vec4(material.colors[1], 1.0f) * vec4(fTexCoord, 1.0f, 1.0f);\n"
		//"	color = texture(material.colors[0], fTexCoord);\n "
		"}\n";
	//----------------------------------------------------------------
